...
## 💰 REVENUE SPLIT — โครงสร้างจัดสรรรายได้ (Core 75/20/5)

> **Confirmed core (ภาพ 18 ส.ค. 2026):** 75% Node Providers / 20% Platform Operations / 5% Developer Pool
> **Core ที่ยืนยันแล้ว:** 75% ผู้ให้เครื่อง / 20% ค่าดูแลระบบ / 5% กองทุนนักพัฒนา

| ผู้รับ / Receiver | สัดส่วน / Share | รายละเอียด / Details |
|---|---|---|
| **Node Providers** (เจ้าของเครื่อง/ร้านเน็ต) | **75%** | จ่ายรายวัน (Daily Earnings) แปลงเป็นเงินบาทท้องถิ่นได้ / Paid daily, THB-convertible |
| **Platform Operations** (ค่าดูแลระบบ) | **20%** | ค่าเซิร์ฟเวอร์ + การบำรุงรักษาระบบ / Servers & maintenance |
| **Developer Pool** (กองทุนนักพัฒนา) | **5%** | Core Code / ปลั๊กอิน / แก้ช่องโหว่ความปลอดภัย / Core code, plugins, vulnerability patches |

Clients pay **USD / Crypto (USDT)** → smart contract splits automatically → node providers receive daily. / ลูกค้าจ่าย **USD / Crypto (USDT)** → สมาร์ทคอนแทร็กต์แบ่งอัตโนมัติ → ผู้ให้เครื่องรับรายวัน
...
## 📋 PROGRAM GOALS — เป้าหมายของโปรแกรม

All program goals, improvement levels (L1 → L3 → ∞) and target dates are defined in **PROGRAM_GOALS.md** (bilingual, DRAFT 20 Aug 2026):

เป้าหมายทั้งหมดของโปรแกรม ระดับการพัฒนา (L1 → L3 → ∞) และกำหนดเวลา อยู่ใน **PROGRAM_GOALS.md** (2 ภาษา ฉบับร่าง 20 ส.ค. 2569):

- **A. Revenue Goals** — 5 ช่องทาง, Arbitrage, รับเงิน 2 แบบ, 75/20/5, เงินบาท
- **B. Workload Orchestration** — รวมพลัง, เทรน AI, ขุด, จัดคิว, ส่งออก 3 ทาง
- **C. Security** — Docker Sandbox, ปกป้องข้อมูล
- **D. Token Economy** — $1/เดือน, Prepaid API, $GRID
- **E. Community & Growth** — 3 กลุ่มผู้ร่วม, บันทึกผู้บุกเบิก, ลีดเดอร์บอร์ด, เครื่องคำนวณ, นักพัฒนา
...
## 🧠 The Open Strategy — กลยุทธ์เปิด (เปิดหมด ไม่ปิดบัง)

> **Why open everything? Because capital can copy documents — but it cannot copy a live network, speed, and execution. We publish our full strategy openly; the moat is the network itself. / ทำไมเปิดหมด? เพราะคนมีเงินลอกเอกสารได้ แต่ลอก "เครือข่ายที่ยังมีชีวิต + ความเร็ว + การลงมือทำ" ไม่ได้ — เราตีพิมพ์กลยุทธ์เต็มรูปแบบต่อสาธารณะ เพราะคู่น้ำคือเครือข่ายเอง ไม่ใช่เอกสาร**

### 1. The Hybrid Network — เครือข่ายไฮบริด
...
- งานราคาถูก → consumer GPU / งานต้องการ SLA → Data Center เล็ก — Arbitrage Engine สลับให้อัตโนมัติ
- พิสูจน์แล้วในตลาด: Vast.ai เองก็ผสม host อิสระกับ certified data center partners
- SLA (Service Level Agreement) = สัญญารับประกันระดับบริการ (uptime 99.9%, latency, recovery) — องค์กรต้องการเพราะเครื่องพัง = เสียเงินจริง — ไฮบริดทำให้เราให้ SLA หลายระดับได้โดยไม่ต้องสร้าง Data Center เอง

### 2. Why we compete with intelligence, not capital — ทำไมแข่งด้วยสมอง ไม่ใช่ทุน

- CoreWeave ชนะด้วยทุน (~$2.1B/ไตรมาส, สัญญา $99B, H100 นับแสน) — เราไม่สู้ในสนามเงิน
- เราชนะด้วย: **Arbitrage Engine** (เลือกงานจ่ายสูงสุดทุกวินาที) + **DisTrO** (เทรนข้ามเครื่อง ลดการสื่อสาร 1,000-10,000 เท่า — งานวิจัยเปิดของ Nous) + **AI Hooks** (ปลั๊กอินโมเดลจูน)
- **ตำแหน่ง:** ถูกกว่า CoreWeave, เสถียรกว่า Vast.ai, ฉลาดกว่าทุกเจ้า

### 3. Lessons from Psyche Network — บทเรียนจากเครือข่าย Psyche (อุปสรรค 4 ข้อ + วิธีแก้)

| อุปสรรค | ปัญหา | วิธีแก้ (พิสูจน์แล้ว) | การประยุกต์ของเรา |
|---|---|---|---|
| คนโกง | ส่งผลขยะ/งานปลอมรับรางวัล | 3 ชั้น: Training/Witnessing/Verifying | Docker Sandbox + คะแนน trust โหนด + จ่ายเฉพาะงานตรวจแล้ว |
| เครื่องเข้าออก | ถอนตัวเมื่อไหร่ก็ได้ | แบ่ง epoch + checkpoint | จ่ายรายวันตามงานจริง (ไม่ทำงาน = ไม่จ่าย) |
| แบนด์วิดท์คอขวด | ส่งข้อมูลช้ากว่าคำนวณ | DisTrO ลด 1,000-10,000 เท่า + quantize DCT | งานระยะแรกเป็นชิ้นอิสระ (inference/เรนเดอร์) ไม่พึ่งเน็ตเร็ว |
| ฮาร์ดแวร์ไม่น่าเชื่อถือ | สเปกต่างกัน ปลอมแปลงได้ | เข้ารหัส end-to-end + NodeId เป็นคีย์ | API key เก็บ hash + โหนดมีตัวตนตรวจสอบได้ |

### 4. Demand Acquisition — วิธีหาลูกค้า (เปิดเผยทั้งหมด)

เรากวาดดีมานด์จากทุกทางพร้อมกัน:

| ครอบครัว | วิธีกวาด | สถานะ |
|---|---|---|
| ตลาดเช่า GPU (Vast.ai/io.net/Render/Akash/Salad) | สมัคร supplier → รับงานอัตโนมัติ | Vast ✅ / io.net+Render ⏳ / อื่น 🔒 |
| บริษัท AI/สตาร์ทอัพ (direct_ai) | Prepaid API + สแกนบริษัทเป้าหมาย | เริ่มได้ |
| มหาวิทยาลัย/นักวิจัย | แพ็กเกจวิจัย + anchor หน่วยงานรัฐ (อว./สวทช./AIT/มธ./KVIS/VISTEC) | เริ่มได้ |
| เกม/สตูดิโอ/เรนเดอร์ (studios) | เรนเดอร์เทรลเลอร์/bot AI/cloud gaming | เริ่มได้ |
| ชุมชน dev ออนไลน์ | Demand Radar: สแกน HN/Lobsters หาคน "กำลังต้องการ" | ✅ ทำงาน |
| ภาครัฐ/โครงการประเทศ | ผ่านเครดิตวิจัย → สัญญาใหญ่ | 6-12 เดือน |
| **ข่าวระดมทุน** | บริษัทเพิ่งได้เงิน = ต้องการ compute ทันที → ยิงตรงก่อนใคร | ✅ ทำงาน (Amity $100M ฯลฯ) |

**หลักการ:** ใครโพสต์ "กำลังหา GPU/ได้เงินก้อนใหม่" = ยิงตรงถึงเขาภายในไม่กี่ชั่วโมง ก่อนคู่แข่ง — ด้วยน้ำเสียง "เพิ่มพลังให้เขา" ไม่ใช่ "ขาย" ("ยินดีด้วยกับการระดมทุน! เราอยากช่วยให้ทีมคุณขยายงาน AI ได้เร็วขึ้น")

### 5. The Open Flywheel — วงล้อที่หมุนเอง

- โมเดลเปิดฟรี (Nous/DeepSeek/Qwen) ปล่อยใหม่ทุกไตรมาส → คนอยากลองแต่เครื่องไม่พอ → เช่าเรา → รายได้ → ขยายเครือข่าย → รองรับโมเดลใหญ่ขึ้น → ดีมานด์เพิ่ม
- ทุกโมเดลฟรีที่ปล่อย = โฆษณาสร้างดีมานด์ให้ตลาดเช่า GPU ฟรีๆ — เราเป็นฝ่ายรับดีมานด์โดยไม่ต้องจ่ายค่าโฆษณา
- เราไม่แข่งโมเดล — เราอยู่ฝั่ง "เครื่อง" — ต่างฝ่ายต่างได้กับผู้สร้างโมเดลเปิด (ผลัก-ดึง)
...
*Version 7 — updated 20 August 2026: full open strategy — hybrid network, compete-by-intelligence positioning, Psyche lessons with application, complete demand acquisition playbook, open flywheel. / เวอร์ชัน 7 — อัปเดต 20 ส.ค. 2569: กลยุทธ์เปิดเต็มรูปแบบ — เครือข่ายไฮบริด, จุดยืนแข่งด้วยสมอง, บทเรียน Psyche พร้อมการประยุกต์, คู่มือหาลูกค้าฉบับสมบูรณ์, วงล้อเปิด*
