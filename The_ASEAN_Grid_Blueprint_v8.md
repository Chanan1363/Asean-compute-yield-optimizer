...
## 🧬 NOUS ALIGNMENT — การวางแนวสถาปัตยกรรมตาม Nous Research (DisTrO / Psyche)

> รายละเอียดเต็ม: `docs/NOUS_ALIGNMENT.md` — อ่านคู่กับเอกสารนี้
> Full detail: see `docs/NOUS_ALIGNMENT.md` — read alongside this blueprint.

**หลักการ (Principles):**
- เราอ้างอิง **ดีไซน์** ของ Nous — ไม่ได้อ้างว่าใช้โค้ดโดยตรง (credit © Nous Research & contributors ตาม license)
- Nous ระบบเป็น **training-first** (DisTrO/Psyche เทรนโมเดลข้ามอินเทอร์เน็ต) — ASEAN Grid เป็น **inference-first** (ขาย compute ผ่าน API) → **complementary ไม่ใช่ competing**

**การวางแนว 2 ชั้น (Two-layer alignment):**

| ชั้น | จุดที่วางตาม | ของจริงใน code เรา |
|---|---|---|
| ⚙️ **เทคนิค** | DeMo optimizer → เติมช่อง `AI_TRAINING` (ลด bandwidth gradient 3-4 เท่าตัวเลข) / Witness-Verify → trust / run-manager+Docker → `DockerSandboxOrchestrator` | `core/` + `ai/strategy_hooks.py` |
| 💰 **เงิน/trust** | points-based + permissionless claim + collateral → `RevenueSplit` v2 (คง EVM + 75/20/5 เป็น pool ratio) | `contracts/RevenueSplit.sol` → v2 |

**Phase ที่เกี่ยวข้อง:** Phase 3 (The Nous Strategic Corridor) — เอกสารนี้คือหลักฐาน "เราเข้าใจและศึกษา Nous อย่างจริงจัง" เมื่อไปติดต่อ
...
*Version 6 — updated 20 August 2026: bilingual TH/EN, new summary heading, confirmed 75/20/5 revenue split, PROGRAM_GOALS reference. / เวอร์ชัน 6 — อัปเดต 20 ส.ค. 2569: 2 ภาษา, หัวข้อสรุปใหม่, สัดส่วนรายได้ 75/20/5 ที่ยืนยันแล้ว, อ้างอิง PROGRAM_GOALS*
...
|---|---|---|
| v7 | 22 ส.ค. 2026 | โครงสร้างหลัก (Blueprint) — architecture, revenue matrix, roadmap |
| **v8** | 23 ส.ค. 2026 | **+ Nous Alignment section** — วางแนวตาม DisTrO/Psyche 2 ชั้น (เทคนิค + เงิน) — ดู `docs/NOUS_ALIGNMENT.md` |
